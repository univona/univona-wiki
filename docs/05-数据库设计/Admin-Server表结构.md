...
| `encryption_policy` | TEXT | NOT NULL, DEFAULT 'optional' | 加密策略：`optional`、`required`、`disabled` |
...
| `sender_id` | UUID | NOT NULL | 发送者 ID |
| `content` | BYTEA | NOT NULL | 加密消息内容 |
| `content_type` | TEXT | NOT NULL, DEFAULT 'encrypted' | 内容类型 |
| `size_bytes` | INTEGER | NOT NULL, DEFAULT 0 | 消息大小（字节） |
